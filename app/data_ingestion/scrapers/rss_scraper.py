...
def fetch_articles_from_rss(rss_url: str) -> list[dict]:
    ...
    try:
        ...
        if feed.bozo:  # Check for malformed feed
            print(
                f"Warning: Feed {rss_url} may be malformed. Bozo reason: {feed.bozo_exception}"
            )
...
def parse_rss_article(article_entry: dict) -> dict:
    ...
    if (
        not content
        and "content" in article_entry
        and isinstance(article_entry["content"], list)
    ):
        # Sometimes content is a list of content objects
        content_parts = [
            c.get("value", "") for c in article_entry["content"] if c.get("value")
        ]
        content = "\n".join(content_parts)
